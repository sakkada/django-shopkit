class PaymentFailure(Exception):
    """Exception which raises on payment failure."""
...
class ConfirmationFormNeeded(Exception):
    """Exception which raises when payment data should be extended."""
...
class RedirectRequired(Exception):
    """Exception which raises when payment method demand redirect."""
...
class PaymentType(object):
    ...
    def __repr__(self):
        return '<PaymentType(provider=%s, typ=%s, name=%s)>' % (
            repr(self.provider), repr(self.typ), repr(self.name)
        )
...
class PaymentProvider(object):
    ...
    def as_choices(self, order=None, customer=None):
        return [(t.typ, t.name)
                for t in self.enum_types(order=order, customer=customer)]
