...
def make_country_action(region):
    ...
    return (name, (action, name,
                   "Add selected countries to '%s' region" % region.name))
...
class DeliveryCountryAdmin(admin.ModelAdmin):
    ...
    search_fields = ('name',)
    fieldsets = (
        (None, {'fields': ('name', 'region', 'promote',)}),
    )
    ordering = ('name',)
    ...
    def get_actions(self, request):
        ...
        actions.update(dict([
            make_country_action(region)
            for region in models.DeliveryRegion.objects.all()
        ]))
        return actions
...
class PostShippingTypeAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {'fields': ('typ', 'name', 'description',),},),
        (None, {'fields': ('region', 'price',),},),
    )
